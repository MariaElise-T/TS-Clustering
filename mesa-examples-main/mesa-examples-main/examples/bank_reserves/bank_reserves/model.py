...
def get_num_rich_agents(model):
    ...
    rich_agents = [a for a in model.agents if isinstance(a, Person) and a.savings > model.rich_threshold]
...
def get_num_poor_agents(model):
    ...
    poor_agents = [a for a in model.agents if isinstance(a, Person) and a.loans > 10]
...
def get_total_savings(model):
    ...
    agent_savings = [a.savings for a in model.agents if isinstance(a, Person)]
...
def get_total_wallets(model):
    ...
    agent_wallets = [a.wallet for a in model.agents if isinstance(a, Person)]
...
def get_total_loans(model):
    ...
    agent_loans = [a.loans for a in model.agents if isinstance(a, Person)]
...
class BankReserves(mesa.Model):
    ...
    def __init__(
        ...
        for i in range(self.init_people):
            ...
            p = Person((x, y), self, True, self.bank, self.rich_threshold)
